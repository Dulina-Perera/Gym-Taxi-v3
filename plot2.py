...
def rl_agent(learning_rate, discount_factor):
    ...
    return total_epochs / num_episodes
...
timesteps_surface = np.zeros((num_points, num_points))

for i in range(num_points):
    for j in range(num_points):
        lr = LR[i, j]
        df = DF[i, j]
        timesteps_surface[i][j] = rl_agent(lr, df)

# Create a 3D plot
ax = plt.axes(projection='3d')
ax.plot_surface(LR, DF, timesteps_surface, cmap='plasma')

# Set labels and title
ax.set_xlabel('Learning Rate')
ax.set_ylabel('Discount Factor')
ax.set_zlabel('Average Timesteps per Episode')
ax.set_title('3D Surface Plot of Average Timesteps per Episode')
